...
Files generated by cloud sessions are written under `~/.nuwa-cli/workspaces/<user_id>/<agent_work_dir>/`. Browse them directly:
...
nuwa-cli also runs a local **file-server** (HTTP, default port `60015`, printed at `serve`/`gateway` startup) that mirrors the same workspace layout and exposes file list / preview / upload / tar endpoints — same paths the engine writes to.
