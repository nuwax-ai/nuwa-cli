...
export const en = {
  ...
  // —— serve ——
  "serve.err.badBool": "Boolean must be true or false, got {value}",
  "serve.err.badPort": "{option} must be an integer 1-65535, got {value}",
  "serve.portTaken":
    "[nuwa-cli] {label} port {preferred} is unavailable; automatically switching to {port}.",
  "serve.daemonNoPid": "daemon child process failed to start: no PID acquired",
  "serve.daemonStarted": "nuwa-cli serve started in the background (pid {pid}).",
  "serve.daemonLog": "Log: {path}",
  "serve.forceStopped":
    "Stopped old serve process(es) via --force: {pids}",
  "serve.started": "nuwa-cli serve started: http://{host}:{port}",
  "serve.secretNoteTunnel":
    "(valid for this process only, not persisted; for local direct-debug you can send this header, and cloud-tunnel requests are authorized by the lanproxy savedKey)",
  "serve.secretNoteLocal":
    "(valid for this process only, not persisted; every request must send this header)",
  "serve.yolo.header": "[nuwa-cli] Auto-approve (auto/yolo) mode is on — note:",
  "serve.yolo.scope":
    "  · ordinary tool calls (file write/delete, command execution, network access) are auto-approved with no path restriction;",
  "serve.yolo.sensitive":
    "  · sensitive access such as local sessions still requires cloud/local approval;",
  "serve.yolo.safety":
    "  · make sure you are listening only on localhost and that X-Nuwax-Internal-Secret is not leaked.",
  "serve.yolo.alt":
    "    For full manual approval use --approve ask; to deny all use --approve deny.",
  "serve.askMode":
    "[nuwa-cli] ask mode: all tool calls go through SSE acpRequestPermission and wait for /computer/notify-resolved approval.",
  "serve.tunnelLoginRequired":
    "[nuwa-cli] --tunnel requires login first: nuwa-cli login --domain <host> --saved-key <key>; this run only provides the local API and will not establish a cloud tunnel.",
  "serve.fileServerPortMismatch":
    "[nuwa-cli] The backend-returned fileServerPort={backend} differs from the CLI's available port {cli}; using the CLI port this run.",
  "serve.lanproxyMissing":
    "Registration succeeded but lanproxy serverHost/serverPort are missing; pass --lanproxy-host and --lanproxy-port",
  "serve.spinner.register": "Registering with Nuwax...",
  "serve.spinner.fileServer":
    "Starting nuwax-file-server and waiting for its health check...",
  "serve.spinner.lanproxy":
    "Starting lanproxy and waiting for the tunnel to come up...",
  "serve.fileServer.started": "nuwax-file-server started (port {port}).",
  "serve.fileServer.unhealthy":
    "[nuwa-cli] nuwax-file-server health check failed (port {port}); file-related endpoints may be unavailable.",
  "serve.lanproxy.started":
    "lanproxy started (pid {pid}, {host}:{port}, ssl={ssl}).",
  "serve.lanproxy.unhealthy":
    "[nuwa-cli] lanproxy health check failed (pid {pid}); the tunnel may not be established — see {log} (daily-rotated; see today's).",
  "serve.tunnel.registerFailed":
    "[nuwa-cli] --tunnel registration failed: {msg}; this run only provides the local API.",
  // —— account ——
  "account.list.empty":
    "No saved accounts. Run `nuwa-cli login` or `nuwa-cli gateway` to add one.",
  "account.list.unknownComputer": "(unknown computer name)",
  "account.switch.running":
    "[nuwa-cli] The Gateway is currently running or did not shut down cleanly (port {port}, PID {pid}). Run `nuwa-cli stop --all` before switching accounts.",
  "account.switch.notFound":
    "[nuwa-cli] Account \"{selector}\" not found. Run `nuwa-cli account list` to see switchable accounts.",
  "account.switch.done":
    "Switched the current account: {username} ({domain}). Please restart `nuwa-cli gateway` or `nuwa-cli serve --tunnel`.",
  "account.switch.failed": "[nuwa-cli] Failed to switch accounts: {msg}",
  // —— workspaces ——
  "workspaces.header": "Workspaces: {root}",
  "workspaces.userLine": "User {user} ({n} projects):",
  "workspaces.row": "  {project}  {fileCount} files  {size}  {time}",
  "workspaces.summary": "\n{n} project directories in total.",
  "workspaces.empty.user": "No project directories under user {user}.",
  "workspaces.empty.all":
    "No workspace directories yet (files generated by cloud sessions after serve/gateway runs are written here).",
  "workspaces.filesUnit": " files  ",
  // —— context / sessions shared ——
  "common.engineMustBeClaudeOrCodex":
    "[nuwa-cli] --engine must be claude or codex",
  "context.list.empty": "No locally referenceable contexts found.",
  "context.err.missingRef": "--ref <engine:sessionId> is required",
  "sessions.searchEmpty": "No sessions match \"{search}\".",
  "sessions.empty": "No local session history found.",
  "sessions.summary.tail":
    "\n{n} local sessions in total. Resume with `nuwa-cli chat --resume`.",
  "sessions.summary.missingSessionId": "[nuwa-cli] --session-id is required",
  "sessions.summary.notFound":
    "[nuwa-cli] sessionId \"{id}\" not found in the local {engine} session history.",
  // —— ui (console) ——
  "ui.err.badPort": "[nuwa-cli] --port must be an integer 1-65535",
  "ui.forceStopped": "Stopped old Console process(es) via --force: {pids}",
  "ui.startFailed": "[nuwa-cli] Console failed to start: {msg}",
  "ui.started": "nuwa-cli console started: {url}",
  "ui.yolo":
    "[nuwa-cli] Auto-approve (yolo) mode: tool calls are auto-approved; sensitive actions still pop up an approval in the browser.",
  "ui.askMode":
    "[nuwa-cli] ask mode: every tool call pops up an approval in the browser.",
  "ui.denyMode":
    "[nuwa-cli] deny mode: all tool calls are rejected (chat only; cannot write files or run commands).",
  "ui.ctrlCExit": "Ctrl+C to exit.",
  // —— chat ——
  "chat.err.contextModesConflict":
    "[nuwa-cli] --resume, --ref-session and --handoff cannot be used together (got: {modes}).",
  "chat.connected":
    "Connected to the {engine} engine (session {id}{resumed}). Type /exit to quit.",
  "chat.resumedSuffix": ", history resumed",
  // —— resolveResumeTarget ——
  "resolve.multipleSelect":
    "Multiple sessions start with \"{option}\"; pick one:",
  "resolve.notFoundId":
    "sessionId \"{id}\" not found in the local {engine} session history. Run `nuwa-cli sessions --engine {engine}` to see available sessions.",
  "resolve.noHistory":
    "No local {engine} session history found; cannot resume.",
  "resolve.autocompleteMessage": "Search or pick a session to resume:",
  "resolve.autocompletePlaceholder":
    "Type a sessionId/keyword to filter...",
  // —— policy ——
  "policy.sensitiveNoChannel":
    "[nuwa-cli] A sensitive tool call needs approval but no approval channel is available; rejected ({reason}). Use serve's SSE/notify-resolved, or --approve deny / interactive chat.",
  "policy.askMessage": "Allow the tool call \"{tool}\"?",
  "policy.badApprove":
    "--approve only supports auto, ask or deny; got \"{raw}\".",
  // —— gateway status line (statusView) ——
  "status.unhealthy": "unhealthy",
  "gateway.lineRunning":
    "Gateway: {state}  http://{host}:{port}  PID {pid}  started {startedAt}",
  "gateway.lineUnhealthy":
    "Gateway: {state}  PID {pid}  http://{host}:{port} (/health not responding; it may still be starting or unhealthy)",
  "gateway.lineStopped": "Gateway: {state}{note} (start with `nuwa-cli gateway`)",
  // —— lanproxy readiness report ——
  "lanproxy.status.presentUnhealthy":
    "[nuwa-cli] A lanproxy process exists (PID {pid}), but Gateway /health is unavailable; see {log}.",
  "lanproxy.status.notDetected":
    "[nuwa-cli] No running lanproxy detected; see {log} or run `nuwa-cli doctor`. Upgrade/install scripts must not judge by spawn success alone; you can run `nuwa-cli start` or `nuwa-cli restart` manually.",
  // —— sensitive access gate ——
  "gate.timeout": "Timed out waiting for sensitive-access approval",
  "gate.deniedByEnv":
    "Sensitive access \"{kind}\" was denied by NUWACLI_SENSITIVE_ACCESS=deny.",
  "gate.noGateway":
    "Sensitive access \"{kind}\" requires local Gateway approval, but no running Gateway was detected. Run nuwa-cli gateway first, or run the command manually in an interactive terminal.",
  "gate.healthFailed":
    "Sensitive access \"{kind}\" requires approval, but the serve health check (port {port}) failed.",
  "gate.noChannel":
    "Sensitive access \"{kind}\" needs an approval channel: open a cloud/local /computer/progress SSE to serve first, then retry.",
  "gate.userDenied": "The user denied sensitive access \"{kind}\".",
  "gate.failed": "Sensitive-access approval failed (HTTP {status})",
  // —— reg client ——
  "reg.timeout": "Request timed out ({ms}ms): {url}",
  "reg.requestFailed": "Request failed: {msg}",
  "reg.httpError": "HTTP {status}: {statusText}",
  "reg.envelopeFailed": "Request failed (error code {code})",
  "reg.error.4010": "Not logged in",
  "reg.error.4011": "Login expired",
  "reg.error.1001": "Client not found",
  "reg.error.9999": "System error",
  // —— core throws / singletons / misc ——
  "common.err.badPort": "port must be an integer 1-65535, got {port}",
  "core.consoleServerError": "[nuwa-cli] Console server error: {msg}",
  "core.serveServerError": "[nuwa-cli] Serve server error: {msg}",
  "context.ref.badEngine":
    "The engine part of a context ref must be claude or codex",
  "context.ref.missingSession": "The context ref is missing a sessionId",
  "singleton.ui.running": "Another Console is already running (PID {pid})",
  "singleton.ui.lockFail": "Failed to acquire the Console singleton lock",
  "singleton.serve.starting":
    "Another serve is starting or running (PID {pid})",
  "singleton.serve.lockFail": "Failed to acquire the serve singleton lock",
  "singleton.serve.ownerChanged":
    "The serve singleton lock owner changed; refusing to start the daemon",
  "processRegistry.stopFailed": "Failed to stop process(es): {pids}",
  "lanproxy.unsupportedPlatform":
    "lanproxy does not support the current platform ({key})",
  "lanproxy.pathNotFound": "--lanproxy-path does not exist: {path}",
  "lanproxy.binaryNotFoundOverride":
    "Could not find {binary} under --lanproxy-path {path} (and no universal fallback). Point it at the Electron resources/lanproxy directory or a single nuwax-lanproxy binary.",
  "lanproxy.binaryNotFound":
    "No lanproxy binary found for the current platform. Re-run npm install (do not use --omit=optional), or specify the binary via --lanproxy-path / NUWACLI_LANPROXY_PATH. Tried: {tried}",
  "engineHome.isolateNoCreds":
    "[nuwa-cli] {name} runs in isolation mode and was given no credentials: it will not reuse {realHome}'s login. Provide them with --api-key/--base-url/--model, or set NUWACLI_ISOLATE_ENGINES=0 to reuse the local login.",
  "engineHome.isolateMigration":
    "[nuwa-cli] codex/claude now runs in isolation mode; old history sessions are still in ~/.codex and ~/.claude (no longer read). To view them, set NUWACLI_ISOLATE_ENGINES=0.",
  "sessionMode.unsupported":
    "[nuwa-cli] The engine does not support mode \"{mode}\"; available: {available}",
  "sessionMode.setFailed":
    "[nuwa-cli] Failed to set mode \"{mode}\": {msg}",
  "ports.allUnavailable":
    "{n} consecutive ports starting from {preferred} are all unavailable",
  "context.ref.badFormat":
    "A context ref should be <engine>:<sessionId>, e.g. claude:xxxxxxxx",
  "context.notFound":
    "sessionId \"{id}\" not found in the local {engine} session history.",
  "singleton.ui.detected":
    "An existing nuwa-cli Console process was detected (PID {pids}). Only one foreground instance is allowed at a time; pass --force to confirm replacement.",
  "singleton.serve.detected":
    "An existing nuwa-cli serve process was detected (PID {pids}). Only one instance is allowed at a time; pass --force to confirm replacement.",
  // skill 子命令
  "cli.cmd.skill.desc": "Manage nuwa-cli skill suite: install (S3 download + link into agent dirs) / update / link / list",
  "cli.cmd.skill.install.desc": "Install skills from S3 (defaults to the nuwa-cli-usage suite entry and its bundled members; auto-symlinks into local agent skill dirs)",
  "cli.cmd.skill.update.desc": "Update skills to S3 latest (re-runs install; same version skipped, --force to override)",
  "cli.cmd.skill.link.desc": "Only (re)link agent skill dirs, no download (wire up an installed skill)",
  "cli.cmd.skill.list.desc": "List installed skills and their per-agent link status",
  "cli.cmd.skill.opt.name": "skill name (defaults to nuwa-cli-usage suite entry)",
  "cli.cmd.skill.opt.version": "pin a version (defaults to latest.json)",
  "cli.cmd.skill.opt.target": "install target dir (defaults to ~/.nuwa-cli/skills; explicit target disables auto-link)",
  "cli.cmd.skill.opt.force": "reinstall even if the same version is present",
  "cli.cmd.skill.opt.noBundle": "skip bundled suite members",
  "cli.cmd.skill.opt.noLink": "skip auto-symlink into local agent skill dirs",
  "skill.scriptMissing": "skills/scripts/install-skill.sh not found (npm package must ship skills/scripts; dev layout resolves from repo root).",
  "skill.scriptSpawnFailed": "Failed to run install-skill.sh: {error}",
  "skill.list.empty": "No skills installed. Install with: nuwa-cli skill install",
  "skill.list.header": "Installed skills (store ~/.nuwa-cli/skills):",
  "skill.list.linkedCount": "linked into {n} agent dirs",
  "skill.list.dirHint": "real dir, not taken over",
  "skill.list.missingRootHint": "agent not installed",
  "skill.list.noneHint": "not linked (run nuwa-cli skill link)",
} as const;
